...
10:00－10:15,図書室,１,スズキ,４,,09:00－09:15,図書室,７３,タナカ,２
...
,美術室,７,,０,,,美術室,７９,,０
,音楽室,８,,０,,,音楽室,８０,,０
10:40－10:55,図書室,９,ヤマダ,３,,09:40－09:55,図書室,８１,,０
,理科室,１０,,０,,,理科室,８２,,０
,美術室,１１,,０,,,美術室,８３,,０
,音楽室,１２,,０,,,音楽室,８４,,０
11:00－11:15,図書室,１３,,０,,10:00－10:15,図書室,８５,,０
,理科室,１４,,０,,,理科室,８６,,０
,美術室,１５,,０,,,美術室,８７,,０
,音楽室,１６,,０,,,音楽室,８８,,０`;
...
function findReservationColumns(headers) {
    // 「予約名」または「予約状況」を含むカラムをすべて特定
    return headers
        .map((header, index) => ({ header, index }))
        .filter(({ header }) => 
            (header.includes('予約名') || header.includes('予約状況') || header.includes('予約')) && 
            !header.includes('代表者') && 
            !header.includes('名簿')
        )
        .map(({ index }) => index);
...
function summarizeReservations(table) {
    ...
    table.rows.forEach((row) => {
        reservationColumns.forEach((nameIndex) => {
            ...
            const isSealed = Boolean(row[nameIndex] && row[nameIndex].trim());
            const parsedCount = toNumber(row[countIndex]);
            const reserved = isSealed ? Math.max(1, parsedCount) : 0;
...
function renderSheetTable(table, isUpdate = false) {
    ...
    const visibleIndexes = table.headers
        .map((_, index) => index)
        .filter((index) => !columnMeta[index].isCount && !columnMeta[index].isHidden && !columnMeta[index].isGap);
    ...
    table.rows.forEach((row) => {
        ...
        visibleIndexes.forEach((index) => {
            ...
            let value = row[index] || '';
            if (columnMeta[index].isReservation && !value.trim()) {
                value = '空白';
            }
...
function getColumnMeta(table) {
    ...
    return table.headers.map((header, index) => {
        const isHidden = header.includes('予約代表者名簿') || header.includes('代表者') || header.includes('〃');
        const isReservation = reservationColumns.has(index);
        // 予約状況カラムは空でも非表示にしない（isGapにしない）
        const isGap = !isReservation && (!header || isHidden || table.rows.every((row) => !(row[index] || '').trim()));
...
function columnClass(tag, meta, index, value = '') {
    ...
    const tone = meta.isReservation
        ? (value && value !== '空白' ? ' reservation-cell text-blood-500 font-bold tracking-widest eerie-shiver' : ' text-gray-600 italic font-mono')
        : meta.isGap
...
function updateStats(data) {
    ...
    countElements.forEach((el) => {
        ...
        if (typeof anime !== 'undefined') {
            anime({ targets: el, innerHTML: [currentVal, target], round: 1, easing: 'easeOutExpo', duration: 1000 });
        } else {
...
function initEerieEvents() {
    function triggerGarble() {
        const textNodes = [];
        document.querySelectorAll('p, h2, h3, span:not(.count-up)').forEach((el) => {
            if (el.textContent.trim().length > 1 && !el.closest('#table-body') && Math.random() < 0.25) {
                textNodes.push(el);
            }
        });
        ...
        if (textNodes.length > 0) {
            ...
            for (let i = 0; i < originalText.length; i++) {
                if (originalText[i] !== ' ' && originalText[i] !== '\n' && Math.random() < 0.45) {
                    ...
                    garbled += originalText[i];
                }
            }

            node.textContent = garbled;
            node.style.color = '#ef4444';
            node.style.textShadow = '0 0 8px rgba(239, 68, 68, 0.8)';

            setTimeout(() => {
                node.innerHTML = originalHTML;
                node.style.color = '';
                node.style.textShadow = '';
            }, 1000); // 1秒間ごちゃごちゃして文字化け
        }

        setTimeout(triggerGarble, 6000 + Math.random() * 8000);
    }

    setTimeout(triggerGarble, 4000);
}

function initHauntedTextDance() {
    const hauntedElements = document.querySelectorAll('.text-haunted-dance, .garble-dance');
    hauntedElements.forEach((el) => {
    ...
    setInterval(() => {
        hauntedElements.forEach((el) => {
            const chars = el.querySelectorAll('.haunted-char');
            if (chars.length === 0) return;
            const randomIndex = Math.floor(Math.random() * chars.length);
            const char = chars[randomIndex];
            char.style.transform = `translateY(${-3 - Math.random() * 4}px) rotate(${(Math.random() - 0.5) * 8}deg)`;
            char.style.color = ['#ef4444', '#fca5a5', '#94a3b8', '#f87171'][Math.floor(Math.random() * 4)];
            char.style.textShadow = `0 0 ${4 + Math.random() * 8}px rgba(239, 68, 68, ${0.3 + Math.random() * 0.5})`;
            setTimeout(() => {
...
function initIntroGarble() {
    const section = document.querySelector('.intro-garble-section');
    if (!section) return;
    const paragraphs = section.querySelectorAll('p');

    function triggerIntroGarble() {
        const p = paragraphs[Math.floor(Math.random() * paragraphs.length)];
        const originalHTML = p.innerHTML;
        const originalText = p.textContent;
        const glitchChars = '!<>-_\\/[]{}—=+*^?#_陰闇刻忌呪歪';
        ...
        p.textContent = garbled;
        p.style.color = '#ef4444';
        p.style.textShadow = '0 0 12px rgba(239, 68, 68, 0.9), 0 0 25px rgba(153, 27, 27, 0.6)';
        p.style.transform = 'translate(' + ((Math.random() - 0.5) * 3) + 'px, ' + ((Math.random() - 0.5) * 3) + 'px)';
        ...
        setTimeout(triggerIntroGarble, 5000 + Math.random() * 6000);
    }

    setTimeout(triggerIntroGarble, 3000);
}
